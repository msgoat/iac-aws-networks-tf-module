...
resource "aws_subnet" "public_web_subnets" {
  ...
  tags = merge({"Name" = "subnet-${data.aws_availability_zones.zones.names[count.index]}-${var.network_name}-web"}, local.module_common_tags)
}
...
resource "aws_subnet" "private_app_subnets" {
  ...
  tags = merge({"Name" = "subnet-${data.aws_availability_zones.zones.names[count.index]}-${var.network_name}-app"}, local.module_common_tags)
}
...
resource "aws_subnet" "private_data_subnets" {
  ...
  availability_zone = data.aws_availability_zones.zones.names[count.index]
  map_public_ip_on_launch = false
  tags = merge({"Name" = "subnet-${data.aws_availability_zones.zones.names[count.index]}-${var.network_name}-data"}, local.module_common_tags)
}
